...
interface XRayTextEffectProps {
  text: string | React.ReactNode;
  color?: string;
  noise?: boolean;
  className?: string;
}
const XRayTextEffect = ({
  text,
  color = "#F43F5E",
  noise = true,
  className,
  ...
  return (
    ...
    >
      ...
      />
      <motion.div
        className={cn("absolute inset-0 w-full border-t-4")}
        style={{ top: overlayTop, borderColor: color }}
      />
      <div className="relative h-full p-4">
        <div className="relative h-full" onMouseMove={handleMouseMove(textY)}>
          <div className="text-5xl font-bold text-white pb-[0.3em]">{text}</div>
          <motion.div
            className="absolute top-0 left-0 text-5xl font-bold text-transparent/85 pb-[0.3em]"
            style={{
